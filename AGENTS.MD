...
**Always use shadcn/ui components** where it makes sense. Avoid native HTML form elements when a shadcn equivalent exists:
...
| Custom dropdowns | `<DropdownMenu>` from `@/components/ui/dropdown-menu` |
| Custom modals | `<Dialog>` from `@/components/ui/dialog` |
| Custom drawers | `<Sheet>` from `@/components/ui/sheet` |

**Available components:** Button, Card, Dialog, DropdownMenu, Input, Label, Select, Separator, Sheet, Switch, Tabs, Textarea, Tooltip, Badge, ScrollArea, Collapsible, Alert

**Adding new UI component:**
```bash
bunx shadcn@latest add [component] --yes
# Component will be created in src/components/ui/[component].tsx
```
